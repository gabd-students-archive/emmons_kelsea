...
This code creates a data frame into a tibble.
```{r}
as_tibble(iris)
```
...
This code creates a tibble with non-syntactic names.
```{r}
tb <- tibble(
  `:)` = "smile",
  `  ` = "space",
...
This code creates a tribble with small amounts of data.
```{r}
tribble(
  ~x, ~y, ~z,
  #--/--/----
...
#### 10.3.1
This code creates a tibble 1000 x 5.
```{r}
tibble(
...
This code creates a data frame with 10 rows and an infinite width.
```{r}
nycflights13::flights %>%
  print(n = 10, width = Inf)
```
These codes limit the rows and or show all rows.
```{r}
options(tibble.print_max = n, tibble.print_min = m)
options(tibble.print_min = Inf)
```
This code shows the complete list of options to use with tibble.
```{r}
package?tibble
```
This code creates a built-in data viewer.
```{r}
nycflights13::flights %>%
  View()
```
#### 10.3.2
This code attaches a piece of a data frame to a variable.
```{r}
df <- tibble(
  x = runif(5),
  y = rnorm(5)
...
```
This code extracts a variable by name.
```{r}
df$x
```
...
df[["x"]]
```
This code extracts the variable by position.
```{r}
df[[1]]
```
These codes uses a placeholder in a pipe.
```{r}
df %>% .$x
```
```{r}
...
To tell if an object is a tibble, you can run `class()` and it will tell you. Or, you can run `str()` and it wil say at the top of the results.
...
The data frame code is more layed out and you can tell what its going to result in, while the tibble code is less code to enter. The data frame code is frustrating because it has so much information to add in that you can't mess up.
...
I would use the `read_delim()` function because it reads files with any delimiter.
...
The two most important arguments with `read_fwf(` is `fwf_widths()`, which specifies the fields widths and `fwf_positions(`) which specifies a fields position.
...
```{r}
This code has more values than designated columns.
read_csv("a,b\n1,2,3\n4,5,6")

This code has more columns than values, also the second row has too many numbers.
read_csv("a,b,c\n1,2\n1,2,3,4")

This code dose not have enough values in the first row.
read_csv("a,b\n\"1")

This code, they went from numerical values to letters.
read_csv("a,b\n1,2\na,b")

This code, they should have ran as a `read_csv2()` because it is separated by semicolons.
read_csv("a;b\n1;3")
```
...
The 1999 and 2000 have to have ticks around them.
```{r}
table4a %>% 
  gather(`1999`, `2000`, key = "year", value = "cases")
...
Between() can select information froma specific variable to another variable.
...
The arrival times also seem to be missing from these flights, so they were probably all cancellations.

**4:** Why is `NA ^ 0` not missing? Why is `NA | TRUE` not missing? Why is `FALSE & NA` not missing? Can you figure out the general rule? (`NA * 0` is a tricky counterexample!)

NA is simply a placeholder for some number. For first problem, any positive number to the power of zero is 1. Same as the TRUE statement. There is a logical value, we just don't know what it is yet, but no matter what it represents, the answer will be the same. Lastly, NA is placeholding for some value, but any value times 0 will equal zero, therefore the answer is always going to be the same.
...
```{r}
select(flights, contains("delay"), contains("time"), -(sched_dep_time), -(sched_arr_time), -(air_time), -(time_hour))
```

**2:** What happens if you include the name of a variable multiple times in a `select()` call?

It will ignore the repeated variable.
...
This function allows you to select variables by their names.
It would be helpful with this vector because there are multiple variables all within one vector.


**4:** Does the result of running the following code surprise you? How do the select helpers deal with case by default? How can you change that default?

The contains() function is trained to ignore the case of the letters, to fix that, you could run:
